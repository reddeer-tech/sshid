...
section "add: creates a key, a record and a fragment"
X add acme --type=ed25519 >/dev/null 2>&1
...
section "add: REFUSES to overwrite an existing key — there is no undo for that"
before=$(shasum -a 256 "$HOME/.skm/acme/id_ed25519" | cut -d' ' -f1)
X add acme >/dev/null 2>&1 && no "second add succeeded" || ok "refused a duplicate name"
rm -f "$HOME/.config/sshid/identities.map"; X list >/dev/null 2>&1   # forget the record, keep the key
X add acme >/dev/null 2>&1 && no "overwrote a key that was not in the manifest" || ok "refused even when only the KEY exists"
[ "$(shasum -a 256 "$HOME/.skm/acme/id_ed25519" | cut -d' ' -f1)" = "$before" ] && ok "the existing key is byte-identical" || no "KEY WAS OVERWRITTEN"
X add acme --existing-key >/dev/null 2>&1 && ok "--existing-key adopts it instead" || no "--existing-key failed"

section "add: name validation happens before anything is written"
for bad in BAD_NAME 'a b' 'x!' '-lead' none global ''; do
  n=$(ls "$HOME/.skm" | wc -l)
  X add "$bad" >/dev/null 2>&1 && no "accepted bad name '$bad'"
  [ "$(ls "$HOME/.skm" | wc -l)" = "$n" ] || no "'$bad' created something"
done
ok "every invalid or reserved name refused, nothing written"

section "route: REFUSES an org with no proof (the 'repository not found' trap)"
X route acme --org github.com:acme-corp >/dev/null 2>&1 && no "routed without proof" || ok "refused without --proof"
grep -q 'acme-corp' "$HOME/.gitconfig" 2>/dev/null && no "wrote a rule anyway" || ok "no rule written"

section "route: a FAILING proof does not route either"
export SSH_REPO_OK=0
X route acme --org github.com:acme-corp --proof git@github.com:acme-corp/app.git >/dev/null 2>&1 \
...
section "route: a PASSING proof routes, and git agrees"
...
section "route --dir covers a repo with no remote at all"
X route acme --dir "$HOME/projects/Acme" >/dev/null 2>&1
...
section "unroute drops one rule and leaves the rest"
X unroute acme --org github.com:acme-corp >/dev/null 2>&1
...
section "unbind keeps the key and the record"
X unbind acme >/dev/null 2>&1
...
X add doomed --existing-key >/dev/null 2>&1
...
section "key types"
X add k-rsa --type=rsa >/dev/null 2>&1
...
X add k-ec --type=ecdsa >/dev/null 2>&1
...
X add beta >/dev/null 2>&1; X route beta --dir "$HOME/projects/Beta" >/dev/null 2>&1
X export > "$T/exp.tsv"
...
X route beta --dir "$HOME/projects/Gamma" >/dev/null 2>&1
